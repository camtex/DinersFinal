...
export const CandidateDashboardPage = ({ profile }: { profile: UserProfile }) => {
  ...
  useEffect(() => {
    ...
    return subscribeToUserApplications(
      profile.uid,
      nextApplications => {
      ...
      error => {
        setErrorMessage(`No pudimos cargar tus postulaciones: ${error.message}`);
      },
    );
  }, [profile.uid]);
  ...
  const handleRefreshAffinities = () => {
    clearStoredQuizTraits();
    setQuizTraits([]);
  };

  const handleRemoveSingleApplication = async (applicationId: string) => {
    try {
      await deleteVacancyApplication(applicationId);
      setFeedbackMessage('Postulacion eliminada correctamente.');
      setErrorMessage('');
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'No pudimos eliminar la postulacion.');
    }
  };
  ...
  const handleSubmitApplication = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFeedbackMessage('Tu postulacion se registro correctamente y ya deberia verse en tu dashboard.');
    } catch (error) {
      setErrorMessage(
        error instanceof Error
          ? `No pudimos registrar la postulacion: ${error.message}`
          : 'No pudimos registrar la postulacion en Firebase.',
      );
    } finally {
        ...
        <div className="container grid gap-8 xl:grid-cols-[0.82fr_1.18fr]">
          <div className="space-y-8">
            <div className="group relative overflow-hidden rounded-[2.5rem] border border-diners-gray-1/30 bg-white p-8 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <span className="text-[9px] font-black uppercase tracking-widest text-[#00A3E0]">Postulante</span>
                    <h2 className="text-xl font-black text-diners-twilight">{fullName}</h2>
                    <p className="text-xs text-diners-twilight-65">{profile.email}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={handleRefreshAffinities}
                    title="Limpiar afinidades"
                  ...
                  <form onSubmit={handleSubmitApplication} className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="h-9 flex-1 rounded-full bg-diners-blue-sky text-xs"
                      >
                        {isSubmittingApplication ? 'Enviando...' : 'Enviar'}
                      </Button>
                      <Button
                        type="button"
                        variant="ghost"
            ...
            {(feedbackMessage || errorMessage) && (
              <Card className={`rounded-[2rem] border p-4 ${errorMessage ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50'}`}>
                <CardContent className={`p-0 text-sm ${errorMessage ? 'text-red-700' : 'text-green-700'}`}>
                ...
                {applications.length > 0 ? applications.map(app => (
                  <div key={app.id} className="group flex items-start justify-between rounded-2xl border border-diners-gray-1/20 bg-white p-4 transition-all hover:border-diners-blue-sky/30">
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => void handleRemoveSingleApplication(app.id)}
